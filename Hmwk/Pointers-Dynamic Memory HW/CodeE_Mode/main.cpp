 ...
 * Created on September 30th, 2020, 8:45 AM
 * Purpose: Find the mode of an array, using
...
int *mode(const int *array,int arySize){
    ...
    int max=0;
    while(i==max){ //Since 0 is first value in all cases, it must be equal to the highest frequency, so only 1 loop is needed
        count=0;
        for(int j=i;j<arySize;j++){ //Count the amount of times the first value shows up
            if(ary[i]==ary[j]){
                count+=1;
            }
        }
    ...
    if(freq>1){
        i=0;
        while(i<arySize){ //While stepping through length of array
            count=0;
            for(int j=i;j<arySize;j++){
                if(ary[i]==ary[j]){    //If value appears again, add 1 to count
                    count+=1;
                }
            }
