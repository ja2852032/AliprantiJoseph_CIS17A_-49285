...
//Overriding function for fleet number
int UsrShips::fleet() const{
    int c;
    //Count all values left, disregarding 0 and 1
    for(int i=0;i<size;i++)
        if(uShip1[i]!=0&&uShip1[i]!=1)
            c++;
    for(int i=0;i<size-1;i++)
        if(uShip2[i]!=0&&uShip2[i]!=1)
            c++;
    for(int i=0;i<size-2;i++)
        if(uShip3[i]!=0&&uShip3[i]!=1)
            c++;
    for(int i=0;i<size-2;i++)
        if(uShip4[i]!=0&&uShip4[i]!=1)
            c++;
    for(int i=0;i<size-3;i++)
        if(uShip5[i]!=0&&uShip5[i]!=1)
            c++;
    for(int i=0;i<size;i++)
        if(cShip1[i]!=0&&cShip1[i]!=1)
            c++;
    for(int i=0;i<size-1;i++)
        if(cShip2[i]!=0&&cShip2[i]!=1)
            c++;
    for(int i=0;i<size-2;i++)
        if(cShip3[i]!=0&&cShip3[i]!=1)
            c++;
    for(int i=0;i<size-2;i++)
        if(cShip4[i]!=0&&cShip4[i]!=1)
            c++;
    for(int i=0;i<size-3;i++)
        if(cShip5[i]!=0&&cShip5[i]!=1)
            c++;
    
    //Return total units left in the game
    return c-3;
}