 ...
 * Created on December 14th, 2020, 4:25 PM
 * Purpose:  Create a game of Battleship between the user and the computer
 * 
 * Final Version: With the program basically complete with a debugged "sunk"
 *                function, all that is left is to incorporate a few more
 *                concepts if and where applicable.
 */
...
int checkR(int); //Error testing with exception
void totLeft(const Ships &); //Display total fleet left
...
int main(int argc, char** argv) {
    ...
    cout<<"To begin the game, there were ";
    totLeft(r);
    cout<<"To end the game, there are ";
    totLeft(a);
...
void uTurn(char tGrid[][COLS],char cGrid[][COLS],UsrShips &a){
    ...
    try{
        //Send row to function to see if it is in range
        t=checkR(r);
    }
    catch(string eString){
        //Catches exception thrown from function since input is faulty
        cout<<eString;
        cin>>r;
        
        //Stay in loop until user gives good input
        while(r<0||r>9){
...
//Function to throw exception if not in range
int checkR(int r){
    if(r<0||r>9){
        string eString = "Not a valid y-coordinate. Please enter a number 0 through 9: ";
        //throw eString to catch()
        throw eString;
    }
    else
        return r; //return as is if in range
}

//Display the fleet started with, and ended with.
void totLeft(const Ships &n){
    cout<<n.fleet()<<" total units on the board."<<endl;
}
