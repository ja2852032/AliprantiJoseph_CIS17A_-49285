 ...
 * Created on December 11th, 2020, 5:45 PM
 * Purpose:  Create a game of Battleship between the user and the computer
 * 
 * Version 1:  Take Project 1, and add onto it with a more "human" guessing
 *             tactic for the computer, where the computer will guess a spot
 *             either above, below, or beside their previous guess, if their 
 *             previous guess was a hit.  This will be created through a class
 */
...
topRow *fillTop(int); //Create top row from a pointer within a structure
...
void plShips(char [][COLS],const int,ifstream &,fstream &); //Place the ships on the grids
...
void play(char [][COLS],char [][COLS],char [][COLS],char [][COLS],topRow *,const int,ComGuess); //Function stepping through game
void uTurn(char [][COLS],char [][COLS]); //Function for user to make their guesses
void cTurn(char [][COLS],char [][COLS],ComGuess &); //Function for computer to make their guesses
...
int sunk(char [][COLS],const int,const int); //Check if a ship has sunk
...
int main(int argc, char** argv) {
    ...
    cout<<"My Grid"<<endl;
    plShips(usrGrid,MAX,inFile,binFile);
    plShips(comGrid,MAX,inFile,binFile);
    prnGrid(usrGrid,topArr,MAX);
    cout<<endl<<"Target Grid"<<endl;
    prnGrid(tarGrid,topArr,MAX);
    cout<<endl;

    play(usrGrid,tarGrid,comGrid,ctGrid,topArr,MAX,cmptr);
    

    //Clean up the code, close files, deallocate memory, etc....
    delete []topArr[n].topGrid;
    delete []topArr;
...
topRow *fillTop(int max){
    ...
    a[f].size=max; //Set size
    a[f].topGrid=new char[a[f].size]; //Set new size for topGrid array
    
    for(int i=A;i<=J;i++){
        getC=revConv(c);
        a[f].topGrid[i]=getC; //Fill topGrid array with chars
        c++;
    }
...
void prnGrid(char grid[][COLS],topRow *arr,const int rows){
    ...
    for(int i=0;i<rows;i++)
        cout<<" "<<arr[n].topGrid[i]; //Format the top x-axis of the grid
    cout<<endl;
    for(int i=0;i<rows;i++){
...
void plShips(char grid[][COLS], const int rows, ifstream &file,fstream &binFile){
    ...
    if(file){
        for(int i=0;i<ships;i++){
            ...
            binFile.write(reinterpret_cast<char *>(&size),sizeof(int)); //Send size of char array
            binFile.write(ship,size*sizeof(char)); //Then send char data to file
            if(size>3){
...
void play(char uGrid[][COLS],char tGrid[][COLS],char cGrid[][COLS],char ctGrid[][COLS],topRow *topArr,const int rows,ComGuess cmptr){
    int res;
    int i=0;
...
void cTurn(char uGrid[][COLS],char ctGrid[][COLS],ComGuess &cmptr){
    int r, //Row input
        c, //Column input
        res, //Get return from reGuess function
        res2, //Get return from sunk function
